    ...
    <main className="auth-page">
      ...
      <section className="auth-card">
        ...
        <Form method="post" className="auth-form">
          ...
          <label>{text("Пароль", "Пароль")}<span className="password-field"><input type={passwordVisible ? "text" : "password"} name="password" autoComplete="current-password" required /><button className="password-field__toggle" type="button" aria-label={passwordVisible ? text("Скрыть пароль", "Приховати пароль") : text("Показать пароль", "Показати пароль")} aria-pressed={passwordVisible} title={passwordVisible ? text("Скрыть пароль", "Приховати пароль") : text("Показать пароль", "Показати пароль")} onClick={() => setPasswordVisible((visible) => !visible)}><svg viewBox="0 0 24 24" aria-hidden="true">{passwordVisible ? <><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.75"/></> : <><path d="M4 10.5c2 2.3 4.7 3.5 8 3.5s6-1.2 8-3.5"/><path d="m7.2 13.5-1.7 2M12 14v2.7M16.8 13.5l1.7 2"/></>}</svg></button></span></label>
